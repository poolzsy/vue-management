<template>
    <div class="home-container">
        ...
        <!-- 批量导入对话框 -->
        <el-dialog v-model="importDialog.visible" title="批量导入用户" width="30%">
...
const importDialog = reactive({
    visible: false,
    uploadUrl: computed(() => `${request.defaults.baseURL}/user/import`),
    templateUrl: computed(() => `${request.defaults.baseURL}/user/import/template`)
});
...
const submitForm = async () => {
    ...
    try {
        ...
        const requestPromise = form.value.id
            ...
            : request.post('/user/save', form.value);
...
const handleDelete = (row) => {
    ElMessageBox.confirm(`确定删除用户 [${row.name}] 吗?`, '危险操作', {
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm('确定批量删除选中的用户吗?', '危险操作', {
        confirmButtonText: '确定删除',
        cancelButtonText: '取消',
        type: 'warning'
    }).then(async () => {
...
const exportData = async () => {
    ...
    const ids = multipleSelection.value.map(item => item.id);
    try {
        const blob = await request.post('/user/export/selected', ids, {
            responseType: 'blob'
        });
        FileSaver.saveAs(blob, `用户信息_${new Date().getTime()}.xlsx`);
    } catch (error) {
